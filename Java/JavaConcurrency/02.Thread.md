...
当调用线程的 interrupted() 方法，会把这个标志符设为 1。后续线程在执行过程中会检查这个标志，进行线程中断。  

但是当线程调用了 wait/sleep/join 等方法，也就是线程不处于 RUNNABLE 状态，会抛出 InterruptedException, 同时把标志重新恢复为 0。
原因：sleep, wait 等方法会不断的检查这个线程的标志，为 true 先将其设置为false，然后直接抛异常
