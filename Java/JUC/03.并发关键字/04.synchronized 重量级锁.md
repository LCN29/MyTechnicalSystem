...
ObjectMonitor * ATTR ObjectSynchronizer::inflate (Thread * Self, oop object) {
  ...
  for (;;) {
    ...
    if (mark->has_locker()) {
      ...
      // 这里将持有的线程数设置为 0, 但是下面设置为膨胀中成功后, 没有设置为 1 ？
      m->OwnerIsThread = 0 ;
      m->_recursions   = 0 ;
...
// displace mark word. inflate 方法必须拷贝在 存锁线程栈的 轻量级锁中的 displace mark word 到 ObjectMonitor, 这样可以保证   
//hashCode 的稳定不变。当前锁的 mark word 是 0, 如果锁拥有的线程决定去释放锁, 释放锁将会失败, 然后走到 slow_exit 方法去膨胀。锁拥有的
// 线程将会进入自旋，等待 markword == 0 消失。
```
