...
> 2. 使用唯一性索引进行等值查询, 没有命中任何一条记录, 这时候是记录锁
> 3. 使用非唯一新索引进行等值查询, 无论是否命中记录, 这时候读锁间隙锁, 锁住的是**当前查询条件的值所在的 2 个记录之间的间隙**
...
Gap Lock 只在 RR 中存在。   
如果要关闭间隙锁, 可以把事务隔离级别设置为 RC, 或者把 innodb_locks_unsafe_for_binlog 设置为 on。  
这时候除了外键约束和唯一性检查会加间隙锁, 其他情况都不会用间隙锁。

#### 3.2.5.3 Next-Key Lock - 临键锁

使用非唯一性索引进行范围查询, 不仅命中了 Record, 还包含了 Gap 间隙, 这时候使用的是临键锁。   

临键锁就是 MySQL 里面默认的行锁算法, 等于 间隙锁 + 记录锁。  

临键锁主要用于非唯一性索引的范围查询。
因为临键锁会进行退化操作
> 1. 使用唯一性索引, 通过等值查询匹配到一条记录, 退化为记录锁
> 2. 使用唯一性索引, 没有匹配到任何记录的时候, 退化成间隙锁



临键锁, 锁住的是范围内记录的上一个区间和下一个区间 锁住最后一个 key 的下一个左开右闭的区间  
...
使用唯一性索引, 临键锁锁住的是**查询范围内, 比范围最小值的前面最接近第一个索引(可以是索引本身) + 比范围最大值的后面最接近的第一个索引(不包含索引本身) 间所有的索引记录 + 索引记录前边的间隙**  
...
> 7. 对于间隙锁的范围, 一般情况锁住的是 **左边是符合条件中主键最大的, 右边则是符合条件中主键最小的**

7 的讲解, 现在有一张表, id 的主键, num 是普通的索引, 表中有数据
...
这时候条件为 `where num = 4`, 最终锁住的范围 (9, 3) 到 (6, 5) 
...
在 RC 中, 一个 update 语句, 如果读到一行记录已经加锁的记录, 此时 InnoDB 返回记录最近提交的版本, 有 MySQL 上层判断此版本是否满足 update 的 where 条件。  
